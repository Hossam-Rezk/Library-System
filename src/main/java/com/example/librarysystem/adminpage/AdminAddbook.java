...
public class AdminAddbook implements Initializable {
    ...
    @FXML
    private TextField publicationyear_input;
    ...
    public void onAddBook(ActionEvent event){
        ...
        String bookPublicationYear = publicationyear_input.getText(); // you must check that it's only 4 dharacters and that all of them are integer cuz it's year
        ...
        if (image_path.isEmpty() || image_path.isBlank()||
            bookName.isEmpty() || bookName.isBlank() ||
            bookAuthor.isEmpty() || bookAuthor.isBlank() ||
            bookPublicationYear.isEmpty() || bookPublicationYear.isBlank() ||
        ...
        Admin.addBook(bookName,bookAuthor,bookCategory,bookPublicationYear,bookStock,bookPrice,image_path);
